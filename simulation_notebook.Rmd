...
j <- 1 # search
m <- 2 # zweiter Klick auf search während schon 2 mal auf display geklickt wurde
```

Likelihood function
```{r}
likelihood <- function(i){
  prod_k <- numeric(1) # wird später mit ergebnis aus k-loop multipliziert
for (k in 1:length(ad_types)){
  prod_l <- numeric(1) # wird später mit ergebnis aus l-loop multipliziert
  ...
  * exp(psi[k] * data_new$purchase[data_new[i,(k+3)] == l]) # erste Zeile
  ...
  prod_l <- prod_l * first_two_lines # Produkt über l
    } # l Ende; loop over total counts
  sum_m <- numeric(0)
    for (m in 0:total_counts[i,5]){ # 0 bis Anzahl Käufe in 120 Tagen
  ...
  prod_k <- (prod_l * third_fourth_line) # Prdoukt aus Zeil 1+2 und Zeile 3-4
  * prod_k 
