...
draw_params <- function() {
  ...
  list(alpha = alpha, beta = beta, psi = psi, theta_mu = theta_mu, Sigma_mu = Sigma_mu, mu = mu)
}
```
...
simulate_user_clickstream <- function(alpha, beta, psi, theta_mu, Sigma_mu, mu) {
  ...
  repeat {
    ...
    if(t > T) return(list(timestamp = c(0, t_j), event = ad_types[c(j_0, j)]))
  }
}

```
Simulated click stream data
```{r}
params <- draw_params()
