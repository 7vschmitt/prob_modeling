...
likelihood <- function(i, params = params){
  ...
  llPsi <- params$psi
  llMu <- params$mu
  llMu <- replicate(n, params$mu) # mu vector 12 000 mal wiederholen f?r Test
  ...
  for (k in 1:length(ad_types)){
    ...
    sum_m <- numeric(0)
      for (m in 0:total_counts[i,5]){ # 0 bis Anzahl Kaeufe in 120 Tagen
        t_m1 <- ifelse(total_counts[i,5] != 0,
                       data_new %>% filter(user ==i & event == 4) %>% 
                         filter(row_number() == m+1) %>% 
                         select(timestamp) %>% 
                         as.numeric(), 
                       0)
        t_m <- ifelse(total_counts[i,5] != 0, 
                      data_new %>% filter(user ==i & event == 4) %>% 
                        filter(row_number() == m) %>% 
                        select(timestamp) %>% as.numeric, 
                      0) 
