...
for (i in 1:100){
  print(likelihood(i, params))
}

library(doParallel)
cl <- makeCluster(2)
registerDoParallel(cl)
system.time({ foreach(i = 1:100, .combine = c, .packages = "dplyr") %dopar% {likelihood(i, params = params)} })[3]

stopCluster(cl)
```


# Likelihood ?berarbeitet
...
likelihood <- function(i, params){
  ...
  prod_k <- 1 # wird spaeter mit ergebnis aus k-loop multipliziert
  for (k in 1:length(ad_types)){
    prod_l <- 1 # wird sp?ter mit ergeb+nis aus l-loop multipliziert
    if (total_counts[i , k+1] != 0){
      first_two_lines <- numeric()
      #for (l in 1:(total_counts[i , k+1])){ 
        # l loop weglassen
