...
draw_params <- function() {
  ...
  # Under the assumption that all psi_ks are independet we can sample with the diagonal of sds, in appendix they calculate psi with the Identity matrix which assumes independence!
  psi <- mvrnorm(mu = psi_mean, 
                 Sigma = psi_sd * diag(nrow = length(psi_sd)))
  # TODO: check if we can use diagonal of sd here 
  theta_mu <- mvrnorm(mu = theta_mu_mean, 
                     Sigma = theta_mu_sd * diag(nrow = length(theta_mu_sd)))
  # TODO: Check how to draw from IW dist. using mean and sd
  #we need the scale matrix S in orer to draw from the IW distribution, v is the degree of freedom and was set to 1 in the appendix (Step 6) 
   # TODO:  ceck how to gte the scale matrix with mean and sd 
  #package for IW: MCMCpack
  # dra samples from:
  riwish(v, S)
