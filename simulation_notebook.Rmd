...
```{r echo=FALSE}
want = c("MASS", "mvtnorm", "tidyverse", "xtable", "MCMCpack", "doParallel", "compositions")
...
data %>%
  ...
  head(n=100)
```
Most ommon click streams with purchases
```{r}
data %>%
  mutate(event_short = recode(event, 'purchase' = 'P', 'other' = 'O', 
                              'search' = 'S', 'display' = 'D')) %>%
...
```{r}
load("mcmc_trajectory")
trajectory[[1]]
params <- trajectory[[1]]

for (i in 1:25){
  print(log_likelihood(i, params))
}

library(doParallel)
cl <- makeCluster(2)
registerDoParallel(cl)
system.time({ foreach(i = 1:25, .combine = c, .packages = "dplyr") %dopar% {log_likelihood(i, params = params)} })[3]
...
```{r}
log_likelihood <- function(i, params){
  llAlpha <- params$alpha
  llBeta <- params$beta
  llPsi <- params$psi
  llMu <- params$mu[i,]
