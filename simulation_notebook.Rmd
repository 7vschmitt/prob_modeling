...
likelihood <- function(i, params){
  ...
  for (k in 1:length(ad_types)){
    ...
    first_line <- llMu[i,k] * # i und k ggf. tauschen, je nach sampling output
      exp(llPsi[k] * data_new$purchase[data_new[i,(k+3)] == l]) # erste Zeile
    sum_j <- numeric(0)
    for (j in 1:(length(ad_types) -1)){ # zweite Zeile erste summe
        sum_index <- data_new %>% filter(user == i & event == k) %>% 
          filter(row_number() ==l) %>% select(j+3) %>% as.numeric()
        sum_m <- numeric(0)
        for (m in 1:sum_index){ # zweite Zeile zweite Summe
          t_l <- data_new %>% filter(user ==i & event==k) %>% 
            filter(row_number() == l) %>% select(timestamp)
          t_m <- data_new %>% filter(user ==i & event==j) %>% 
            filter(row_number() == m) %>% select(timestamp)
          tmp_m <- llAplha[j,k] * exp(-llBeta[j] * (t_l - t_m)) # zweite Zeile
          sum_m <- tmp_m + sum_m
          } # m Ende
    ...
    for (j in 1:(length(ad_types) -1)){ # letzte Zeile erste Summe
      ...
      for (m in 1:total_counts[i,j+1]){ # letzte Zeile zweite Summe
        ...
        tmp_m <- (llAplha[j,k]/llBeta[j]) * (1-exp(-llBeta[j] * (120 - t_m)))
        sum_m <- sum_m + tmp_m # Summe m
      } # m Ende
